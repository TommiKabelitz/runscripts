...
from colarunscripts import configIDs
...
def FullCheck(inputArgs: dict, *args, **kwargs):
    ...
    for kappa in jobValues['kappaValues']:
        ...
        print(start,ncon)
        cfgIDs = [configIDs.ConfigID(i,jobValues['runPrefix'],start) for i in range(1,ncon+1)]
        print("Checking IDs")
        missingList = []
        for ID in cfgIDs:
            jobValues['cfgID'] = ID
        ...
        for kd in jobValues['kds']:
            for structure in jobValues['structureList']:
                for sinkType in jobValues['sinkTypes']:
                    tarPath,_ = GetTarFile(parameters,kd,jobValues['shifts'][0],sinkType,jobValues,structure)
                    for chi,chibar in jobValues['particleList']:
                        ...
                        cfgFileList = tarPath.replace('CHICHIBAR',f'{chi}{chibar}')+'cfglist'
...
def Input():
    ...
    parser.add_argument('-f','--fullcheck',help="Whether to fully check for file existence or to just check id list",action='store_true')
    parser.add_argument('-p','--parametersfile',help='The parameters file to use. Default is ./parameters.yml.',default='./parameters.yml')
    parser.add_argument('-o','--outputfile',help='The output file to write to. Default is ./missingCfuns.txt.',default='./missingCfuns.txt')
