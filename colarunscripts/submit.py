...
def SubmitJobs(parameters, nthConfig, inputArgs, values, *args, **kwargs):
    ...
    if inputArgs["skipDeleteCheck"] is False:
        ...
        if values["keepEmodes"] is parameters["tempStorage"]["lapmodes"] is False:
            print(
                "WARNING: proceeding will result in the deletion of locally stored eigenmodes."
            )
    ...
    for kappa in values["kappaValues"]:
        ...
        # Scheduling jobs
        ScheduleJobs(
            filename, values["scheduler"], values["doArrayJobs"], ncon, inputArgs
        )
...
def MakeSlurmRunscript(
    ...
    **kwargs,
    ...
    schedulerDetails["OUTPUTDIR"] = dirs.FullDirectories(
        parameters, directory="stdout"
    )["stdout"]
    ...
    if testing == "testqueue":
        ...
        if "test" not in out.stdout:
            raise ValueError(
                'Test queue does not configured on your machine. Try "-t headnode" or "-t interactive".'
            )
        else:
        ...
        raise ValueError(
            'Interactive jobs not presently supported by slurm. Try "-t headnode".'
        )
    ...
    **kwargs,
    ...
    if testing in ["testqueue", "interactivetestqueue"]:
        ...
        if "express" not in out.stdout:
            raise ValueError(
                'Test queue does not exist on your machine. Try "-t headnode" or "-t interactive".'
            )
        else:
