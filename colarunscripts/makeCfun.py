...
def main(parameters, kd, shift, jobValues, timer, *args, **kwargs):
    ...
    filestub = (
        dirs.FullDirectories(parameters, directory="cfunInput")["cfunInput"]
        + jobValues["jobID"]
        + "_"
...
def MakeCorrelationFunctions(
    parameters, filestub, kd, shift, jobValues, timer, *args, **kwargs
):
    """
...
def MakeReusableFiles(
    parameters, filestub, logFile, kd, shift, jobValues, *args, **kwargs
):
    """
    ...
    # file
    configFile = dirs.FullDirectories(parameters, directory="configFile", **jobValues)[
        "configFile"
    ]
    files.MakeGFSFile(
        filestub,
        logFile,
        parameters["directories"]["configFormat"],
        configFile,
        FormatShift(shift, fullShift="full"),
    )

    # Making the smearing file, still read in for Laplacian sink (I think)
    files.MakePropSmearingFile(
...
def MakeSpecificFiles(
    ...
    modeFiles = dirs.LapModeFiles(
        parameters, kd=kd, shift=shift, quark=structure, **jobValues
    )  # (dict)
    lapModeFiles = [modeFiles[quark] for quark in structure]  # (above as list)
    files.MakeLPSinkFile(
        filestub,
        logFile,
        shift=FormatShift(shift, fullShift="lpsink"),
        lapModeFiles=lapModeFiles,
        **parameters["sourcesink"],
    )
    ...
    for chi, chibar in jobValues["particleList"]:
        ...
        files.MakeInterpFile(
            partstub,
            jobValues["inputSummary"]["interp"],
            chi,
            chibar,
            structure,
            cfunPrefix,
...
def HadronicProjection(parameters, kd, structure, *args, **kwargs):
    ...
    if kd == 0:
        ...
        details["fullLandauFile"] = dirs.FullDirectories(
            parameters, directory="landau"
        )["landau"]
        return details
...
def TarCfuns(
    ...
    """
    tarPath, cfunBase = GetTarFile(
        parameters, kd, shift, sinkType, jobValues, structure
    )
    ...
    for chi, chibar in jobValues["particleList"]:
        ...
        if len(cfunList) == 0:
            print(
                f"No cfuns for {chi}{chibar}. If you are not debugging, something has gone wrong"
            )
            continue
...
def GetTarFile(
    ...
    **kwargs,
    ...
    cfunFiles = dirs.GetCfunFile(
        parameters,
        jobValues["kappa"],
        kd,
        shift,
        jobValues["sourceType"],
        sinkType,
        sinkVal,
        jobValues["cfgID"],
    )
...
def CreateTar(
    tarPath: str, cfunList: list, shift: str, jobValues: dict, *args, **kwargs
) -> None:
    """
