...
def main(parameters, kd, shift, jobValues, timer, *args, **kwargs):
    ...
    filestub = (
        "prop" + jobValues["jobID"] + "_" + str(jobValues["nthConfig"]) + ".QUARK"
    )

    # Compiling the full configuration file
    jobValues["configFile"] = dirs.FullDirectories(
        parameters, directory="configFile", kd=kd, **jobValues
    )["configFile"]
    ...
    for structure in parameters["runValues"]["structureList"]:
        ...
        for quark in structure:
            ...
            propPath = MakePropagator(
                parameters, quark, kd, shift, jobValues, filestub, logFile, timer
            )
...
def MakePropagator(
    parameters, quark, kd, shift, jobValues, filestub, logFile, timer, *args, **kwargs
):
    """
    ...
    quarkPrefix = dirs.FullDirectories(
        parameters,
        directory="prop",
        kd=kd,
        shift=shift,
    ...
    fullQuarkPath = (
        f'{quarkValues["quarkPrefix"]}kKAPPA.{parameters["directories"]["propFormat"]}'
    )
    ...
    filestub = dirs.FullDirectories(parameters, directory="propInput")[
        "propInput"
    ] + filestub.replace("QUARK", quark)

    # Making input files
    MakePropInputFiles(
        parameters, filestub, logFile, quarkLabel, kd, shift, quarkValues
    )
    ...
    reportFile = dirs.FullDirectories(
        parameters,
        directory="propReport",
        kd=kd,
        shift=shift,
        **quarkValues,
        **parameters["sourcesink"],
    )["propReport"].replace("QUARK", quark)
    if quark in ["s", "nh"]:
...
def MakePropInputFiles(
    parameters, filestub, logFile, quarkLabel, kd, shift, quarkValues, *args, **kwargs
):
    """
    ...
    quarkValues["sourcetype_num"] = files.MakeSourceFile(
        parameters, filestub, logFile, kd, shift, quarkValues
    )
